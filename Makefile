...
synth:
	...
	python3 scripts/parser.py --n $(N) --data_w $(DATA_W) --sum_w $(SUM_W) --seed $(SEED) --indir $(OUTDIR) --dataflow $(DATAFLOW)
...
resweep:
	for df in ws os; do \
		for n in 2 4 8 12 16; do \
			for dw in 4 8 16; do \
				python3 scripts/parser.py --n $$n --data_w $$dw --sum_w $(SUM_W) --seed $(SEED) --dataflow $$df --indir $(OUTDIR); \
			done; \
		done; \
	done 
