...
## 2. RISC-V Core
- ALU (alu.sv): A 32-bit Arithmetic Logic Unit supporting standard RISC-V operations (ADD, SUB, AND, OR, XOR, Shifts, and Set-Less-Than).

- Register File (regfile.sv): A 32-word by 32-bit synchronous memory block with two asynchronous read ports and one synchronous write port, adhering strictly to the RISC-V x0 zero-register specification.
...
```
python3 scripts/generate_array_tests.py
```
3. Compile and run the simulation using Verilator
```
iverilog -g2012 -o sim/sim_array tb/tb_systolic_array.sv rtl/systolic_array.sv rtl/pe.sv
vvp sim/sim_array
```
